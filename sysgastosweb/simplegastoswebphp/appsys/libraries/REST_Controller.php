...
abstract class REST_Controller extends CI_Controller {
	...
	*/
	public function __construct($config = 'rest')
	{
		...
		// imported Format for codeigniter powered at venenux, currently part of CI3
	...
	*/
	protected function _check_php_session()
	{
		...
		if ( ! $this->session->userdata($key))
		{
			...
			$this->response(array(
			$this->config->item('rest_status_field_name') => FALSE,
			$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_unauthorized')	), self::HTTP_UNAUTHORIZED);
		}
	}
	...
	*/
	protected function _prepare_digest_auth()
	{
		...
		if (strcasecmp($digest['response'], $valid_response) !== 0)
		{
			...
			$this->response(array(
			$this->config->item('rest_status_field_name') => FALSE,
			$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_invalid_credentials')), self::HTTP_UNAUTHORIZED);
		}
	}
	...
	*/
	protected function _check_blacklist_auth()
	{
		...
		if (preg_match($pattern, $this->config->item('rest_ip_blacklist')))
		{
			...
			$this->response(array(
			$this->config->item('rest_status_field_name') => FALSE,
			$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_ip_denied')), self::HTTP_UNAUTHORIZED);
		}
	}
	...
	*/
	protected function _check_whitelist_auth()
	{
		...
		if (in_array($this->input->ip_address(), $whitelist) === FALSE)
		{
			$this->response( array(
			$this->config->item('rest_status_field_name') => FALSE,
			$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_ip_unauthorized')), self::HTTP_UNAUTHORIZED);
		}
	}

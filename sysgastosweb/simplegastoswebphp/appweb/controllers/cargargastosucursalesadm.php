...
class cargargastosucursalesadm extends CI_Controller {
	...
	public function gastomanualfiltrarlos()
	{
		...
		if ( $this->nivel == 'especial' )
			$sqlentidad .= "  AND cod_entidad <> '111' AND cod_entidad <> 113 and cod_entidad <> 1009 and cod_entidad <> 176 and cod_entidad <> 121 and cod_entidad <> 212 and cod_entidad <> 1109 ";
		...
		if ( ! in_array("edit", $urlsegmentos) or ! in_array("add", $urlsegmentos) )
		{
			...
			$this->db->trans_begin();	// en una tabla temporal solo registros ultimos y por perfil
				$sqltablagastousr = "
					...
					if ( $this->nivel != 'administrador' and ($fec_conceptoini != '' or $fec_conceptofin != '') ) 	$sqltablagastousr .= "AND CONVERT(SUBSTRING(fecha_concepto,1,6),UNSIGNED) >= CONVERT('".(date('Ym')-1)."',UNSIGNED)";

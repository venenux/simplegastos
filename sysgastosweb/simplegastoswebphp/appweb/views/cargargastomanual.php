	...
	else if ($accionejecutada == 'gastomanualcargaruno')
	{
		...
		$this->table->set_template(array ( 'table_open'  => '<table border="0" cellpadding="0" cellspacing="0" class="table">','cell_start' => '<td class="form-field-box odd">', ) );
			...
			$this->table->add_row('Monto (punto para decimal, sin coma)', form_input('mon_registro','', $classinput).br().PHP_EOL);
			$this->table->add_row('Concepto o Detalle:', form_input('des_concepto','', $classinput).br().PHP_EOL);
			$this->table->add_row('Factura tipo:', form_dropdown('tipo_gasto', $tipo_gasto , $classinput));
			$this->table->add_row('Factura Numero (contribuyente):', form_input('factura1_num','', $classinput).br().PHP_EOL);
			$this->table->add_row('Factura RIF (contribuyente):', form_input('factura1_rif','', $classinput).br().PHP_EOL);

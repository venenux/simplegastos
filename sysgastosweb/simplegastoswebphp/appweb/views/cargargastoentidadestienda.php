...
/*		$this->table->clear();
			$this->table->add_row('Filto Fecha desde '.$valoresinputfechaini.' hasta:'$valoresinputfechafin.br().PHP_EOL);
			$this->table->add_row('Por Categoria/Concepto:'.$cod_subcategoriaver.br().PHP_EOL);
			$this->table->add_row('Por Centro de Costo:'.$cod_entidadver.br().PHP_EOL );
			$this->table->add_row('Monto menor o igual'.$mon_registroigual.'y/o mayor o igual'.$mon_registromayor.br().PHP_EOL);
			$this->table->add_row('Por Concepto :'.$des_registrolike.br().PHP_EOL);
		echo $this->table->generate();
*/		if( isset($css_files) )
